...
|---|---|---|---|
| Your personal workspace | created on first login (`POST /v1/auth/sync`) | **Free** (default) | Free onboarding, Base dashboard, locked actions |
| The seeded demo org | `make seed-demo` (`scripts/seed_demo.py`, org `is_demo=true`) | **Pro** (intentional, so the demo shows applied savings) | Pro dashboard with populated, real seeded savings |
| Any org you flip | operator plan switch (below) | Free ⇄ Pro | switching the *same* workspace between tiers |
...
5. Reload the dashboard. The nav badge flips between `Base` and
   `Pro`, and locked actions unlock.
...
- Sign in as a fresh user → land in `/start` → `/onboarding`. Create the workspace,
  ...
  received" with cost/tokens/model/latency/request id and the Base framing.

### Base dashboard
- Nav shows `Base` + `Upgrade`. Side nav shows the Base plan card.
...
- Savings panels are empty / $0 ("No savings recorded yet"). **There must be no
  captured-savings numbers** — Base disables caching, so a repeated request
...
### Free locked actions
- Apply on a recommendation is disabled with "Enable Pro to apply…".
...
- On the demo org (or your org flipped to Pro), nav shows `Pro`.
...
- The demo org is **intentionally Pro** and carries seeded savings. If a
  dashboard looks "Free but with savings," check the nav badge — you are probably on
  the demo (Pro) org. Use your personal Free org (or flip a tier) to test the
  true Free experience.

## Automated coverage

- `tests/test_entitlements.py` — gates + `/v1/entitlements` shape (free vs performance).
- `tests/test_observe_only.py` — Free does not cache/capture savings; Pro does;
