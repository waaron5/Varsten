...
export type CreateFn = (body: any) => Promise<any>;
...
export async function executeWithFallback(params: ExecuteParams): Promise<any> {
  ...
  const doFallback = async (
    ...
    const res = await params.fallbackCreate(params.body);
  ...
  if (params.mode === "off") {
    return params.primaryCreate(params.body);
  }

  // Breaker open: skip Varsten entirely and go direct.
  if (!params.breaker.allow()) {
    return doFallback("breaker_open", undefined, "pre-request");
  }

  try {
    const res = await params.primaryCreate(params.body);
    ...
    return doFallback(decision.reasonCode, decision.varstenStatus, "pre-request", requestId);
  }
}
