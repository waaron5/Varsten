...
def build_demo(
    ...
    *,
    ...
    for i in range(WINDOW_DAYS):
        ...
        for j in range(n_cache):
            ...
            if j < n_hit:
                ...
                events.append(
                    _event(
                        project=project, org_id=org.id, api_key_id=api_key.id, model=INCUMBENT,
                        in_tok=in_tok, out_tok=out_tok, cost=Decimal("0"),
                        latency_ms=rng.randint(3, 14),
                        metadata={"proxy": True, "cache": "hit", "naive_cost_usd": str(full), "saved_usd": str(full)},
                        ts=ts, idempotency_key=_key(),
                    )
                )
                if ts >= m_start:
                ...
                events.append(
                    _event(
                        project=project, org_id=org.id, api_key_id=api_key.id, model=INCUMBENT,
                        in_tok=in_tok, out_tok=out_tok, cost=full,
        ...
        for j in range(n_route):
            ...
            if j < n_control:
                ...
                events.append(
                    _event(
                        project=project, org_id=org.id, api_key_id=api_key.id, model=INCUMBENT,
                        in_tok=in_tok, out_tok=out_tok, cost=inc_cost,
                        latency_ms=rng.randint(700, 1200),
                        metadata={
                            "proxy": True, "cache": "miss", "holdback": True, "arm": ARM_CONTROL,
                            "experiment_from": INCUMBENT, "experiment_to": CANDIDATE, "quality_ok": ok,
                        },
                        ts=ts, idempotency_key=_key(),
                    )
                )
                ...
                events.append(
                    _event(
                        project=project, org_id=org.id, api_key_id=api_key.id, model=CANDIDATE,
                        in_tok=in_tok, out_tok=out_tok, cost=cand_cost,
                        latency_ms=rng.randint(420, 760),
                        metadata={
                            "proxy": True, "cache": "miss", "routed": True,
                            "routed_from": INCUMBENT, "routed_to": CANDIDATE,
                            "naive_cost_usd": str(inc_cost), "saved_usd": str(saved),
                            "holdback": True, "arm": ARM_TREATMENT,
                            "experiment_from": INCUMBENT, "experiment_to": CANDIDATE, "quality_ok": ok,
                        },
                        ts=ts, idempotency_key=_key(),
                    )
        ...
        for _ in range(n_plain):
            ...
            events.append(
                _event(
                    project=project, org_id=org.id, api_key_id=api_key.id, model=INCUMBENT,
                    in_tok=in_tok, out_tok=out_tok, cost=_cost(INCUMBENT, in_tok, out_tok),
                    latency_ms=rng.randint(700, 1200),
                    metadata={"proxy": True, "cache": "miss"},
                    ts=ts, idempotency_key=_key(),
                )
    ...
    batch_saved_month = _build_batches(
        db, events, project=project, org_id=org.id, api_key_id=api_key.id,
        day_start=day_start, m_start=m_start, rng=rng, key_fn=_key,
    )
    ...
    _write_lever(db, org, project, "cheaper_model", routing_saved_month, "approve",
...
def _build_batches(
    ...
    *,
    ...
    for offset in (26, 19, 12, 5, 1):
        ...
        events.append(
            _event(
                project=project, org_id=org_id, api_key_id=api_key_id, model=INCUMBENT,
                in_tok=in_tok, out_tok=out_tok, cost=actual_cost, latency_ms=None,
                metadata={
                    "proxy": True, "batch": True, "lever": "batching",
                    "naive_cost_usd": str(sync_cost), "saved_usd": str(saved),
                },
                ts=created, idempotency_key=key_fn(),
            )
        )
        if created >= m_start:
